      ...
      .subscribe({
        next: (res) => {
          ...
          if (res?.success === false && res?.needEmailConfirmation) {
            ...
            this.successMessage =
              res?.messageAr || res?.message || 'حسابك غير مؤكد. تم إرسال رسالة تأكيد إلى بريدك الإلكتروني.';
            return;
          }
          ...
          // Fallback: رسالة عامة
          this.errorMessage =
            res?.messageAr || res?.message || 'Login failed, please check your credentials';
        },
        error: (err) => {
          // 401 أو غيره
          this.errorMessage =
            err?.error?.messageAr || err?.error?.message || 'Login failed, please check your credentials';
        }
      });
  }
  ...
  resendConfirmation() {
    ...
    this.authService.resendConfirmation(email)
      ...
      .subscribe({
        next: (res) => {
          this.successMessage =
            res?.messageAr || res?.message || 'تم إرسال رسالة التأكيد مرة أخرى إلى بريدك.';
        },
        error: (err) => {
          this.errorMessage =
            err?.error?.messageAr || err?.error?.message || 'تعذر إعادة إرسال رسالة التأكيد.';
        }
      });
  }
}
